...
#     time_series_gather.append(openbb.economy.macro(parameters = [f'{elem}'], countries = ['Greece',], symbol='EUR')[0].droplevel(0, axis='columns'))
#     units_gather.append(openbb.economy.macro(parameters = [f'{elem}'], countries = ['Greece',])[1])
#     denomination_gather.append(openbb.economy.macro(parameters = [f'{elem}'], countries = ['Greece',])[2])


# for i, elem in enumerate(time_series_gather):
#     try:
#         print(f"{i} : {elem.columns[0]} : {parameters_df.iloc[i,0]} [{units_gather[i]}, {denomination_gather[i]}]")
#         df = elem.copy()
#         fig = px.line(df, x=elem.index, y=elem.columns[0], title=f'{parameters_df.iloc[i,0]} - {parameters_df.iloc[i,1]}')
#         fig.layout.update(showlegend=True, template="simple_white", title_x=0.5, plot_bgcolor='white',width=800, height=500)
...
st.markdown("#")
indicators = list(parameters_df.index.unique()).append('All')
indicators_to_plot_ = st.multiselect("Select the indicatos to plot", list(parameters_df.index.unique()), help='Please select the indicators you want to plot - you can select as many as you want or "All" to plot all the indicators')
